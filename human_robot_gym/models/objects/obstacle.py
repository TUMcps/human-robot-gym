...
from human_robot_gym.utils.spatial import homogeneous, rotX, skew, NO_TRANSLATION, NO_ROTATION, NEUTRAL_HOMOGENEOUS
...
class ObstacleBase(ABC):
    ...
    def __copy__(self):
        ...
        """
        warnings.warn(
            "{}'s copy behavior defaults to deepcopy. Explicit call to deepcopy recommended!".format(self.__class__)
        )
...
class MeshObstacle(ObstacleBase):
    ...
    def __deepcopy__(self, memodict={}):
        return self.__class__(self.name, self.mesh_geometry.clone(), self.T[:3, 3], self.T[:3, :3])

    @property
    def as_dict(self) -> Dict[str, any]:
        if len(self.mesh_file) == 0:
            raise ValueError("Cannot store mesh obstacle if original filepath is unknown")
        return {
            'pose': self.T.tolist(),
            'file': self.mesh_file,
...
class Plane(ObstacleBase):
    ...
    def visualize(self, viz: pin.visualize.MeshcatVisualizer):
        ...
        viz_plane = meshcat.geometry.Box(np.array([10, 10, .01]))
        viz.viewer[self.name].set_object(viz_plane)
        viz.viewer[self.name].set_transform(R @ homogeneous(translation=self.d) @ self.T)
...
class ComposedObstacle(ObstacleBase):
    ...
    def __init__(self,
        ...
        super().__init__(name, list(itertools.chain(*[child.collision_objects for child in obstacles])))

    def __copy__(self):
        """Shallow copies all children IF they have a shallow copy. For obstacles, copy defaults to deepcopy"""
        return self.__class__(self.name, [copy(o) for o in self._children], self.T[:3, 3], self.T[:3, :3])

    def __deepcopy__(self, memodict={}):
        return self.__class__(self.name, [deepcopy(o, memodict) for o in self._children], self.T[:3, 3], self.T[:3, :3])
...
def crok2obstacle(geometry: Dict[str, any],
                  package_dir: Path,
                  name: str,
    ...
    rotation = pose[:3, :3]
    if "file" in geometry:
        scale = np.asarray(geometry.get('scale', [1., 1., 1.]), dtype=float)
        if scale.size == 1:
            scale = np.ones((3,), dtype=float) * scale
        try:
            relative_path = geometry['file']
            if relative_path.startswith('/'):  # Commonly used in urdf-like formats, but incompatible with pathlib
                relative_path = relative_path[1:]
            filepath = str(package_dir.joinpath(relative_path))
        except AttributeError:
            raise ValueError("You must provide a package dir when a mesh file is given.")

        mesh_loader = hppfcl.MeshLoader()
        mesh = mesh_loader.load(filepath, scale)
        return MeshObstacle(name, mesh,
                            translation=translation, rotation=rotation,
                            mesh_file=relative_path, scale=scale)
...
def hppfcl2obstacle(name: str, fcl: hppfcl.CollisionObject) -> ObstacleBase:
    ...
    if isinstance(geom, hppfcl.Box):
        return Box(name, *[2*hs for hs in geom.halfSide], translation=translation, rotation=rotation)
    elif isinstance(geom, hppfcl.Cylinder):
        return Cylinder(name, geom.radius, 2 * geom.halfLength, translation=translation, rotation=rotation)
